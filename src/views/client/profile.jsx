...
const Profile = () => {
  ...
  const [artist, setArtist] = useState(null); // État pour stocker l'artiste
  ...
  useEffect(() => {
    const fetchArtist = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/artistes/${id}/details`);
        setArtist(response.data);
        setLoading(false);
      } catch (err) {
        setError('Erreur lors de la récupération des données de l\'artiste.');
        setLoading(false);
      }
    };
  ...
  // Slider settings for discography carousel
  ...
  return (
    <div className="max-w-[1920px] mx-auto scroll-smooth bg-[#FFFFFF]">
      {/* Bouton Go Back */}
          ...
          <SocialIcon  href={artist.instag}  target="_blank"className="hover:bg-gradient-to-r hover:from-[#f9ce34] hover:via-[#ee2a7b] hover:to-[#6228d7] transition-all duration-300 ">
        ...
        <SocialIcon href={artist.Facebook} target="_blank"   className="hover:bg-[#3b5998] transition-colors duration-300">
    ...
    {`${artist.prenom} ${artist.nom}`.toUpperCase()}
      ...
      <div style={styles.tabs}>
        ...
        <button
          ...
          style={{
            ...styles.tabButton,
            ...(activeSection === 'profile' ? { color: 'red' ,   textDecoration: 'underline' // Ajoute une soulignure
            } : {}),
          }}
        >
          Profile
      ...
      {activeSection === 'main' && (
        <div style={styles.profileContainer}>
          <img
            src={
              artist.photoCouverture && artist.photoCouverture.length > 0
                ? `${process.env.REACT_APP_API_BASE_URL.replace('/api', '')}/${artist.photoCouverture[0]}`
                : '/default-avatar.png'
            }
            alt="Cover"
        ...
        <img
          src={
            artist.photo && artist.photo.length > 0
              ?`${process.env.REACT_APP_API_BASE_URL.replace('/api', '')}/${artist.photo[0]}`
              : '/default-avatar.png'
          }
          alt="Profile"
...
{`${formatDate(artist.dateDeJoindre)} `}          </p>
    ...
    __html: highlightName(
      ...
      artist.prenom,
      artist.nom
    ) , __html: formatText(`${artist.profile}`) 
  }} />
          </p>
...
{activeSection === 'discography' && (
 ...
 {selectedChanson && (
  ...
  <strong></strong> {formatDate(selectedChanson.anneeDeCreation)}
              ...
              <p style={{marginBottom:'6px'}}><strong>{selectedChanson.nom} </strong> </p>
...
<footer className="bg-[#111] py-12 px-10 font-sans tracking-wide pt-10">
